...
public class GUIController {
  ...
  /**
   * Constructor that takes in various arguments and sets them to the appropriate
   * variables
   * 
   ...
   * @param select     SelectionFrame responsible for when the user is selecting a
   *                   showtime.
   * @param theatreCon TheatreController responsible for accessing the list of
   *                   theatres.
   * @param payCon     PaymentController responsible for payments.
   * @param ticCon     TicketController responsible for booking and cancelling
   *                   tickets.
   * @param theUesr1   User object representing the customter.
   * @param dbc        DataBaseController responsible for communicating to and
   *                   from the database.
   */
  ...
  /**
   * Inner class responsible for managing functionalities of the registration
   * frame
   * 
   * @author Michael Vassilev, Minh Vo, Matthew Wells, Junhao Xue
   */
  class RegisterListener implements ActionListener {
    ...
    @Override
    /**
     * If the register button is clicked, use the information entered to create a
     * new user
     */
    public void actionPerformed(ActionEvent e) {
      if (e.getSource() == regis.getRegister()) {
        theUser.register(regis.getFirstName() + regis.getLastName(), regis.getAddress(), regis.getEmail(),
            regis.getPhone(), regis.getCardType(), regis.getCard(), regis.getExpiryDate());
  ...
  class PaymentListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      /**
       * If the confirm button is clicked, proceed to make a payment and display an
  ...
  /**
   * Inner class responsible for managing functionalities of the cancel ticket
   * frame
   * 
   * @author Michael Vassilev, Minh Vo, Matthew Wells, Junhao Xue
   */
  class CancelListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      /**
       * If the confirm button is clicked, cancel the ticket and receive an email with
       * credit
       */
      else if (e.getSource() == c.getConfirm()) {
        // The system day is November 9 due to data base construction
        if (DBControl.getTicket(Integer.parseInt(c.getTicketNumber())).getTheShowtime().getDate().getMonth() == 11
            && DBControl.getTicket(Integer.parseInt(c.getTicketNumber())).getTheShowtime().getDate().getDay() > 12) {
  ...
  class SelectionListener implements ActionListener {
    ...
    /**
     * Selection Listener keeps track of the current chosen theatre, movie, showtime
     * and seat
     */
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      /**
       * If the apply button next to theatre is clicked, set current theatre to the
       * chosen theatre. Then populate the list of movies this theatre has. If the
       * user is a registered user, also display pre-order movies.
       */
      ...
      /**
       * If the apply button next to movie is clicked, set current movie to the chosen
       * movie. Then populate the list of showtimes this movie has
       */
      ...
      /**
       * If the apply button next to showtime is clicked, set current showtime to the
       * chosen showtime. Then populate the list of seats this showtime has. If the
       * movie is a pre-order movie, only display 10% of seats
       */
      ...
      /**
       * If the apply button next to seat is clicked, set current seat to the chosen
       * seat, display the cost of the seat.
       */
