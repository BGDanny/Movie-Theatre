/**
 * Class responsible for managing all operations relating to customer tickets.
 * It contains variables allowing communication to the other controller classes,
 * and methods managing the booking and cancelation processes for tickets.
 * 
...
public class TicketController {
  ...
  /**
   * Constructor that takes in a DataBaseController object and sets it to
   * variables DBControl.
   * 
  ...
  /**
   * Books a ticket based on a previously built ticket. Also resets theCredit and
   * theTicket to null so that they are not reused. Finally updates the database
   * with all relevant information and returns a string holding the text of an
   * email the user will recieve.
   * 
   * @param emailAddress email address to send the payment receipt to.
   * @param totalCost:   the total cost that the user is being charged after
   *                     credit has been applied.
   * @return String holding all the information necessary to send a receipt to the
   *         user.
   */
  public String bookTicket(String emailAddress, double totalCost) {
    ...
    String ticketInfo = theTicket.getTheTheatre().getName() + " " + theTicket.getTheMovie() + " "
        + theTicket.getTheShowtime().toString() + " " + theTicket.getTheSeat().emailString() + '\n' + "at a price of: "
        + totalCost;
  ...
  /**
   * Takes in the customers email address and the ID of a ticket that is to be
   * cancelled. It then gets the information about the ticket from the database,
   * gets the cost of the ticket, and creates a credit with the appropirate value.
   * The method then removes the ticket form the data base, adds the credit to the
   * database, and sends an email to the customer with the credit number. The user
   * will be able to use this credit number the next time they purchase a ticket
   * to get a discount.
   * 
